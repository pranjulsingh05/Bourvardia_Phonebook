...
from Phonebook_App.helper_function import authentication, start_session
from Phonebook_App.logging import login_logger
        ...
        if serializer.is_valid():  # and request.method == 'POST' :
            try:
                ...
                if client_id == info['id'] and email == info['email']:
                    serializer.save()
                    arr = [UserDesignation, UserLocation, UserLanguage,
                           UserSkill, UserProject, HierarchyClient]
                    for i in arr:
                        serializers = i(data=request.data)
                        if serializers.is_valid():
                            serializers.save()
                    if start_session(email_id=email, token=token):
            ...
            try:
                info = authentication(token_id=token_id)
                if client_id == info['id'] and email == info['email'] and start_session(email_id=email, token=token):
                    login_logger.info('Successful Login by %s.', email)
                    content = {'Note': 'Successful Login.'}
