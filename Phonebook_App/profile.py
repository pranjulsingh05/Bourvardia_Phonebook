...
from Phonebook_App.logging import profile_logger
...
#
#         Parameter :
#         request : This is the request method which is used to
#         perform function upon the database for the particular user.
#         pk : This is the primary key which is used to identify
#              the particular user upon which the functions will be appiled.
#         :return:
...
class AddManager(APIView):
    ...
    def put(self, request, pk):
        """
            ...
            Parameter :
            request : This is the request method which is used
                    to perform function upon the database for the particular user.

            pk : This is the primary key which is used to
                identify the particular user upon which the functions will be appiled.
        ...
        if client_id != 'null':
            if request.method == 'PUT':
                hierarchyserializer = HierarchySerializers(client_id, data=request.data)
                if hierarchyserializer.is_valid():
                    hierarchyserializer.save()
                    content = {'Manager Added Successfully.'}
                    return Response(content, status=status.HTTP_200_OK)
...
class UserUpdateView(APIView):
    ...
    def get(self, request, pk):  # API to retrieve and update details of logged in user.
        """
            This function is used get all the data of the users from the database.
        ...
        if session_manage(self=token):
            try:
                ...
                hobbies = Employee.objects.get(pk=pk)
            except ValueError:
            ...
            if client_id != 'null':
                if request.method == 'GET':
                    ...
                    hobbiesserializer = EmpSerializers(hobbies, context={'request': request})
                    ...
                    profile_logger.error("Method Should be GEt")
                    return Response(status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, pk):
        """
            This function is used update all the data of the users in the database.
            ...
            request : This is the request method which is used to perform function upon the database for the particular user.
            pk : This is the primary key which is used to identify the particular user upon which the functions will be appiled.
            :return:A status code response upon editing or updating the data of the user with the requested primary key.
        ...
        if session_manage(self=token):
            ...
            if client_id != 'null':
                if request.method == 'PUT':
                    ...
                    hobbiesserializer = EmpSerializers(hobbies, data=request.data, context={'request': request})
                    try:
                        ...
                        if gitserializer.is_valid():
                            ...
                            content = {'error': 'Check the GitSerializer Input Field'}
                            return Response(content, status=status.HTTP_400_BAD_REQUEST)
                        ...
                        skill_set = list(skill.values())
                        skills = ", ".join(skill_set)
                        skillserializer = EmpSkillSerializer(skillid, data={"skill": skills})
                        ...
                        if hobbiesserializer.is_valid():
                            hobbiesserializer.save()
                        else:
                            profile_logger("HobbiesSerializer Error.")
                            content = {'error': 'Check The Hobbies Input Field.'}
                            return Response(content, status=status.HTTP_400_BAD_REQUEST)
...
#              identify the particular user upon which the functions will be appiled.
#
#         :return: A status code response upon editing or
#                  updating the data of the user with the requested primary key.
#         """
#     token = request.META.get('HTTP_AUTHORIZATION')
#     if session_manage(self=token):
#         try:
